...
import { LucideCreditCard, LucideDollarSign, LucideBarChart2 } from "lucide-react";

const slides = [
  {
    title: "Gestión de cuentas unificada",
    description: "Conecta todas tus cuentas bancarias, tarjetas e inversiones en un solo lugar para una visión completa.",
    icon: <LucideCreditCard className="w-6 h-6 text-purple-500" />,
  },
  {
    title: "Ahorra hasta un 30%",
    description: "Con nuestras recomendaciones personalizadas",
    icon: <LucideDollarSign className="w-6 h-6 text-green-500" />,
  },
  {
    title: "+15M de usuarios",
    description: "Confían en Finzen para sus finanzas",
    icon: <LucideBarChart2 className="w-6 h-6 text-yellow-500" />,
  },
];
        ...
        <div className="w-1/2 flex flex-col justify-center px-16 text-white">
          ...
          <div className="bg-white/10 backdrop-blur-xs bg-opacity-10 rounded-2xl p-6 max-w-md">
            <div className="flex items-center gap-4 mb-2">
              {slides[current].icon}
              <h2 className="text-white font-semibold">{slides[current].title}</h2>
            </div>
            <p className="text-sm text-gray-200">{slides[current].description}</p>
