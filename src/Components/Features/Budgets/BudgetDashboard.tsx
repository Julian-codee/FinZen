...
export default function BudgetDashboard() {
  ...
  const fetchBudgets = useCallback(async () => {
    ...
    try {
      ...
      const mappedCategories: BudgetCategoryUI[] = data.map((budget) => ({
        ...
        associatedEntityId:
          budget.cuenta?.idCuenta ||
          budget.tarjeta?.idTarjeta ||
          budget.inversion?.idInversion,
        associatedEntityType: budget.cuenta
          ? "cuenta"
          : budget.tarjeta
          ? "tarjeta"
          : budget.inversion
      ...
      setError(
        err.message || "Ocurrió un error desconocido al cargar presupuestos."
      );
  ...
  const handleUpdateBudget = async (
    id: string,
    budgetAmount: number,
    currentCategory: BudgetCategoryUI
  ) => {
    setIsOperationLoading(true);
    const token = localStorage.getItem("token");
    ...
    try {
      if (!currentCategory.originalCategoryId) {
        throw new Error(
          "ID de categoría original no disponible para la actualización."
        );
      }
      ...
      if (
        currentCategory.associatedEntityType === "cuenta" &&
        currentCategory.associatedEntityId !== undefined
      ) {
        updatedBudgetDto.idCuenta = currentCategory.associatedEntityId;
      } else if (
        currentCategory.associatedEntityType === "tarjeta" &&
        currentCategory.associatedEntityId !== undefined
      ) {
        updatedBudgetDto.idTarjeta = currentCategory.associatedEntityId;
      } else if (
        currentCategory.associatedEntityType === "inversion" &&
        currentCategory.associatedEntityId !== undefined
      ) {
